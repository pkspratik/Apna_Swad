...
export function AdminDashboard() {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "orders"), (snapshot) => {
      ...
      list.sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0)); // newest on top
      setOrders(list);
    });
    return () => unsubscribe();
  }, []);
        ...
        <tbody>
          {orders.map((o, i) => (
            ...
            >
              ...
              <td>
                <button className="delete-btn" onClick={() => handleDelete(o.id)}>❌</button>
              </td>
            </tr>
          ))}
        </tbody>
