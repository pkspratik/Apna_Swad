...
export function Cart() {
  ...
  const [deliveryAvailable, setDeliveryAvailable] = useState(null);
  const [userDistance, setUserDistance] = useState(null);
  const [deliveryCheckedOnce, setDeliveryCheckedOnce] = useState(false); // ⬅ NEW
  ...
  const handleCheckDelivery = () => {
    ...
    setChecking(true);
    setDeliveryCheckedOnce(true); // ⬅ Disable button after click
  ...
  return (
    <div className="cart-wrapper">
      ...
      <button
        className="checkout-btn"
        onClick={handleCheckDelivery}
        disabled={checking || deliveryCheckedOnce} // ⬅ disabled after 1 click
        style={{
      ...
      {cart.length === 0 ? (
        ...
        <>
          ...
          <div className="cart-footer">
            ...
            {/* Proceed shows only when distance ≤ 30 KM */}
            {deliveryAvailable === true && (
              <button className="checkout-btn" onClick={handleCheckout}>
                Proceed to Checkout
              </button>
